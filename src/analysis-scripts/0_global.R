...
input_data_path <- file.path(dirname(dirname(getwd())),'inputs')
output_data_path <- file.path(dirname(dirname(getwd())),'outputs')
auxiliary_database_name <- 'data.duckdb'
auxiliary_database_path <- file.path(input_data_path, auxiliary_database_name)
log_path <- file.path(dirname(dirname(getwd())),'outputs','logs')
      ...
      tryCatch(
        {
          ...
          dbExecute(con,"CREATE OR REPLACE VIEW cohort_view AS SELECT *, FALSE as flag_violating_val, FALSE as flag_listwise_del FROM person;")
          ## Log info
